...
def privateKeyToWif(key_hex):
    # key_hex is a hex string; convert to bytes and encode with base58-check using prefix 0x80
...
def derSigToHexSig(s):
    ...
    s_bytes, junk = ecdsa.der.remove_sequence(s_bytes)
    if junk != b'':
        print('JUNK', junk.hex())
    assert(junk == b'')
...
# Input is a hex string representing the private key.
...
def pubKeyToAddr(s):
    # s is expected to be a hex string representing the public key.
...
def addrHashToScriptPubKey(b58str):
    ...
    # The script: OP_DUP (76), OP_HASH160 (a9), push 20 bytes (14 in hex),
    # then the 20-byte hash, then OP_EQUALVERIFY (88) and OP_CHECKSIG (ac)
...
class TestKey(unittest.TestCase):
    ...
    def test_der(self):
        expected = "300b020312345602040089abcd"
        encoded = ecdsa.der.encode_sequence(
    ...
    def test_derSigToHexSig(self):
        ...
        expected = "4c01fee2d724fb2e34930c658f585d49be2f6ac87c126506c0179e6977716093faad0afd3ae536cfe11f83afaba9a8914fc0e70d4c6d1495333b2fb3df6e8cae"
        self.assertEqual(derSigToHexSig(derSig), expected)
        ...
        # Remove the last byte (hashtype) from the DER signature before conversion.
        ...
        self.assertTrue(vk.verify_digest(bytes.fromhex(sig), bytes.fromhex(hashToSign)))
